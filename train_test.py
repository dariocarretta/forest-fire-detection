...
def get_data(type:str, data_dir:str='/home/dario/Desktop/FlameSentinels/', batch_size:int=16):
    ...
    Returns
        ...
        - only a single DataLoader, num_channels (e.g., test_loader) otherwise
...
def evaluate_model(model, test_loader, device, output_dir='/home/dario/Desktop/FlameSentinels/TEST_PRED_IMGS'):
    ...
    idxs = [i for i in range(len(test_loader))]
    id = 0
    ...
    with torch.no_grad():
        for x, y in tqdm(test_loader, desc="Testing"):
            ...
            for j, pred in enumerate(preds):
                ...
                img.save(os.path.join(output_dir, f'prediction_{idxs[id]*len(preds)+j}.png'))
...
test_loader, num_channels = get_data('test', batch_size=1)
...
unet_model.load_state_dict(torch.load('/home/dario/Desktop/FlameSentinels/best_prediction_model.pth'))

evaluate_model(unet_model, test_loader, 'cuda')